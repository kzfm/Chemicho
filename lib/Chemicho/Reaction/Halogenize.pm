package Chemicho::Reaction::Halogenize;
...
Perhaps a little code snippet.
    ...
    my $cvt = Chemicho::Halogenize->new();
...
sub react {
    ...
    my @halogen = ("Cl","F","Br");
    my $symbol = $halogen[rand(@halogen)]; # choice symbol
    my $h = $react->new_atom(symbol => $symbol);
    $react->new_bond(atoms => [$h, $nfg[0]], order => '1');
    $h->implicit_hydrogens(0);
